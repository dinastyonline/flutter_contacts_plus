...
import com.dinastyonline.flutter_contacts_plus.Account as PAccount
import com.dinastyonline.flutter_contacts_plus.Address as PAddress
import com.dinastyonline.flutter_contacts_plus.Email as PEmail
import com.dinastyonline.flutter_contacts_plus.Event as PEvent
import com.dinastyonline.flutter_contacts_plus.Group as PGroup
import com.dinastyonline.flutter_contacts_plus.Name as PName
import com.dinastyonline.flutter_contacts_plus.Note as PNote
import com.dinastyonline.flutter_contacts_plus.Organization as POrganization
import com.dinastyonline.flutter_contacts_plus.Phone as PPhone
import com.dinastyonline.flutter_contacts_plus.SocialMedia as PSocialMedia
import com.dinastyonline.flutter_contacts_plus.Website as PWebsite
...
class ContactsService {
    companion object {
        ...
        fun select(
            ...
            if (query == null && !config.withProperties && !config.withThumbnail && !config.withPhoto &&
            ...
            if (config.withAccounts || !config.returnUnifiedContacts) {
                projection.addAll(
                        listOf(
                                Data.RAW_CONTACT_ID,
            ...
            val contactsGroups = if (config.withGroups) fetchGroups(resolver) else mapOf<String, PGroup>()
            ...
            var contacts = linkedMapOf<String, Contact>()
            if (cursor == null) {
                return listOf()
            }
            ...
            while (cursor.moveToNext()) {
                // ID and display name.
                val id = if (config.returnUnifiedContacts) getString(Data.CONTACT_ID) else getString(Data.RAW_CONTACT_ID)
                val foundContact = contacts[id];
                var photo: ByteArray? = foundContact?.photo;
                var thumbnail: ByteArray? = foundContact?.thumbnail;
                var name: PName? = foundContact?.name;
                var nickname: String? = null;
                var accounts = foundContact?.accounts ?: mutableListOf();
                var phones = foundContact?.phones ?: mutableListOf();
                var emails = foundContact?.emails ?: mutableListOf();
                var addresses = foundContact?.addresses ?: mutableListOf();
                var organizations = foundContact?.organizations ?: mutableListOf();
                var websites = foundContact?.websites ?: mutableListOf();
                var socialMedias = foundContact?.socialMedias ?: mutableListOf();
                var events = foundContact?.events ?: mutableListOf();
                var notes = foundContact?.notes ?: mutableListOf();
                var groups = foundContact?.groups ?: mutableListOf();

                // Fetch high-resolution photo if requested.
                if (config.withPhoto && foundContact != null) {
                    val contactUri: Uri =
                ...
                // Thumbnails.
                if (config.withThumbnail && mimetype == Photo.CONTENT_ITEM_TYPE) {
                    thumbnail = cursor.getBlob(cursor.getColumnIndex(Photo.PHOTO))
                }
                ...
                if (config.withProperties) {
                    if (config.withAccounts) {
                        ...
                        val found = accounts.any { it?.rawId == rawId }
                        if (!found) {
                            accounts += PAccount(
                                    rawId,
                                    accountType,
                            ...
                            accounts = accounts.map { if (it?.rawId == rawId) PAccount(
                                rawId,
                                accountType,
                                accountName,
                                (it.mimetypes + mimetype).toSet().toList()
                            ) else it }
                        }
                    }
                    ...
                    when (mimetype) {
                        StructuredName.CONTENT_ITEM_TYPE -> {
                            var nick = ""
                            if (foundContact?.name?.nickname != null) {
                                nick = foundContact.name.nickname
                            }
                            name = PName(
                        ...
                        Nickname.CONTENT_ITEM_TYPE ->
                            nickname = getString(Nickname.NAME)
                        ...
                        Event.CONTENT_ITEM_TYPE -> {
                            ...
                            if (YYYY_MM_DD matches date) {
                                year = date.substring(0, 4).toLong()
                                month = date.substring(5, 7).toLong()
                                day = date.substring(8, 10).toLong()
                            } else if (MM_DD matches date) {
                                month = date.substring(2, 4).toLong()
                                day = date.substring(5, 7).toLong()
                            }
                        ...
                        GroupMembership.CONTENT_ITEM_TYPE -> {
                            if (config.withGroups) {
                                ...
                                if (contactsGroups.containsKey(groupId)) {
                                    groups += contactsGroups[groupId]!!
                                }
                            }
                        }
                    }
                ...
                var contact = Contact(
                        ...
                        thumbnail,
                        photo,
                        phones,emails,addresses,organizations,websites,socialMedias,events,notes,accounts,groups
                );
        ...
        fun insert(
            ...
            val insertedContacts: List<Contact> = select(
                    resolver,
                    ContactsQueryByRawId(rawId.toString()),
                    ContactsRequest(
                            withProperties = true,
                            withThumbnail = true,
                            withPhoto = true,
        ...
        fun update(
            ...
            // Load contacts with that raw ID, which will give us the full contact as it
            // was saved.
            val updatedContacts = select(
                    resolver,
                    ContactsQueryByRawId(rawContactId),
                    ContactsRequest(
                            withProperties = true,
                            /*withThumbnail=*/true,
                            withPhoto = true,
        ...
        fun getGroups(resolver: ContentResolver): List<PGroup> {
            val groups = fetchGroups(resolver)
            return groups.values.toList()
        }

        fun insertGroup(resolver: ContentResolver, group: PGroup): PGroup {
            val ops = mutableListOf<ContentProviderOperation>()
        ...
        fun updateGroup(resolver: ContentResolver, group: PGroup): PGroup {
            val ops = mutableListOf<ContentProviderOperation>()
        ...
        fun deleteGroup(resolver: ContentResolver, group: PGroup) {
            val ops = mutableListOf<ContentProviderOperation>()
        ...
        fun openExternalPickOrInsert(activity: Activity?, context: Context?, insert: Boolean, contact: Contact?) {
            if (activity == null && context == null) return
        ...
        private fun getQuick(resolver: ContentResolver, includeNonVisible: Boolean): List<Contact> {
            // This drops contacts not part of any group.
            // See: https://stackoverflow.com/questions/28665587/what-does-contactscontract-contacts-in-visible-group-mean-in-android
            ...
            while (cursor.moveToNext()) {
                contacts.add(
                        Contact(
                                ...
                                null, null, null, mutableListOf(), mutableListOf(), mutableListOf(), mutableListOf(), mutableListOf(), mutableListOf(), mutableListOf(), mutableListOf(), mutableListOf(), mutableListOf()
                        )
                )
            }
        ...
        private fun getPhoneLabel(cursor: Cursor): String {
            return when (cursor.getInt(cursor.getColumnIndex(Phone.TYPE))) {
        ...
        private fun getEmailLabel(cursor: Cursor): String {
            return when (cursor.getInt(cursor.getColumnIndex(Email.TYPE))) {
        ...
        private fun getWebsiteLabel(cursor: Cursor): String {
            return when (cursor.getInt(cursor.getColumnIndex(Website.TYPE))) {
        ...
        private fun buildOpsForContact(
            ...
            for ((i, phone) in contact.phones.filterNotNull().withIndex()) {
            ...
            for ((i, email) in contact.emails.filterNotNull().withIndex()) {
            ...
            for (address in contact.addresses.filterNotNull()) {
            ...
            for (organization in contact.organizations.filterNotNull()) {
                ops.add(
                        newInsert()
            ...
            for (website in contact.websites.filterNotNull()) {
            ...
            for (socialMedia in contact.socialMedias.filterNotNull()) {
            ...
            for (event in contact.events.filterNotNull()) {
            ...
            for (note in contact.notes.filterNotNull()) {
            ...
            for (group in contact.groups.filterNotNull()) {
                ops.add(
                        newInsert()
